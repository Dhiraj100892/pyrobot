...
class GetCameraExtrensics(object):
    ...
    def optimize_ransac(self, num_iter, rot_loss_w, num_ransac_iter, inlier_thr, vis):
        ...
        for n in range(num_ransac_iter):
            ...
            best_train_loss, best_train_quat_G_C, best_train_trans_G_C = None, None, None

            ###################
            # optimize on the train set the B<-->C & G<-->A
            for it in range(num_iter):
                _, train_loss = compute_loss(train_trans_B_G, train_rot_B_G, train_trans_C_A, train_rot_C_A, trans_G_C,
                                             quat_G_C, rot_loss_w)
                optimizer.zero_grad()
                train_loss.backward()
                optimizer.step()
                ...
                if best_train_loss is None or train_loss.item() < best_train_loss:
                    ...
                    best_train_quat_G_C = quat_G_C.detach().numpy()
                    best_train_trans_G_C = trans_G_C.detach().numpy()
            ...
            with torch.no_grad():
                test_loss, _ = compute_loss(test_trans_B_G, test_rot_B_G, test_trans_C_A, test_rot_C_A,
                                            torch.from_numpy(best_train_trans_G_C),
                                            torch.from_numpy(best_train_quat_G_C),
                                            rot_loss_w)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gripper_frame', help='robot gripper frame name', type=str, default="/gripper")
    parser.add_argument('--camera_frame', help='camera frame name', type=str, default="/kinect2_rgb_optical_frame")
    parser.add_argument('--num_data_points', help='number of data points used to optimize the intial guess', type=int,
                        default=5)
