...
class Slam(object):
    def __init__(self, robot, robot_name, map_size, resolution, robot_rad, agent_min_z, agent_max_z,
        ...
        if self.robot_name == "locobot":
            from slam.util.locobot_bumper_check import BumperCallbacks
...
def main(args):
    ...
    if args.robot == 'habitat':
        ...
        path_to_habitat_scene = os.path.join(os.path.realpath(".."), "habitat")
    ...
    slam = Slam(robot, args.robot, args.map_size, args.resolution, args.robot_rad, args.agent_min_z, args.agent_max_z,
                args.vis, args.save_vis, args.store_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--resolution", help="per pixel resolution of map in cm", type=int, default=5)
    ...
    parser.add_argument("--store_path", help="path to store visualization", type=str, default='./tmp')
